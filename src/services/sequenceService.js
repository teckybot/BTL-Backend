...
// Get the **next sequence number** without incrementing in DB (used for ID preview or generation)
export async function getNextSchoolSequence(stateCode, districtCode) {
  ...
  // Return the next number (existing value + 1) but do NOT increment DB yet
  return counter ? counter.sequence_value + 1 : 1;
}

// Get current sequence without increment (for checking only)
...
// Increment **after** school is successfully registered
